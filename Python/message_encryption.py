...
# Create a new dictionary to store the encrypted messages
encrypted_message_data = message_data.copy()

for person, messages in message_data.items():
    for message in messages:
        encrypted_message = encrypt_message(message["message"], shared_secret_key)
        # Store the encrypted message in the new dictionary
        encrypted_message_data[person][0]["message"] = encrypted_message.hex()

print("Encrypted message_data dictionary:")
print(encrypted_message_data)
